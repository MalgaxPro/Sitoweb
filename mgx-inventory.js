...
(function(){
  ...
  // Inject CSS
  if(!document.getElementById('mgxInvStyles')){
    const style = document.createElement('style');
    style.id = 'mgxInvStyles';
    style.textContent = `
      .mgx-inv-mask{position:fixed;inset:0;background:rgba(0,0,0,.45);opacity:0;pointer-events:none;transition:opacity .2s;z-index:9800}
  ...
  if(document.getElementById('mgxInvDrawer')) return;
  ...
  const closeB = document.createElement('button');closeB.id='mgxInvClose'; closeB.className='mgx-inv-close'; closeB.type='button'; closeB.textContent='Chiudi';
  ...
  // React to popup login/logout messages
  window.addEventListener('message', (ev)=>{
    const d = ev.data||{};
    if(d.type==='login'){ if(document.body.classList.contains('mgx-inv-open')) loadInv(); }
